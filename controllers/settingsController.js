...
exports.updatePhoneNumber = async (req, res) => {
  ...
  try {
    const user = await User.findByIdAndUpdate(userId, { number: newPhoneNumber }, { new: true });
    res.json({ success: true, message: 'Phone number updated successfully', user });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Failed to update phone number', error: error.message });
  }
};
...
exports.deleteAddress = async (req, res) => {
  ...
  try {
    const user = await User.findByIdAndUpdate(userId, { $pull: { address: { _id: addressId } } }, { new: true });
